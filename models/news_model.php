...
class News_model extends CI_Model { 
	...
	//修改编辑内容
	function  editnews($news_id,$news_array){
		$this->db->where('id',$news_id);
		$result = $this->db->update($this->tname,$news_array);
	...
	/*获取编辑文章ID的选项*/
	function getone_bycatid($cid){
		$salStr = "SELECT catname FROM $this->tname1 WHERE catid=$cid";
		$query = $this->db->query($salStr)->row_array();
